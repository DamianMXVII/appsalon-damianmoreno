...
function iniciarApp(){
    ...
    paginaSiguiente();
    paginaAnterior();
    consultarAPI(); //CONSULTA LA API DE PHP EN EL BACKEND
...
function mostrarSeccion(){
    //Ocultar la seccion que tenga mostar
    const seccionAnterior = document.querySelector('.mostrar');
    if(seccionAnterior){
            seccionAnterior.classList.remove('mostrar')

    }
    //Sleccionar la seccion con el paso
...
function seleccionarServicio(servicio){
    const {servicios} = cita; //AQUI SE HACE DESTRUCTURACION
    const {id} = servicio;
    const divServicio = document.querySelector(`[data-id-servicio="${id}"]`);

    //Comprovar si un servicio ya fue agregado
...
function mostrarResumen(){
    ...
    servicios.forEach(servicio => {
        ...
        precioServicios.innerHTML = `<span>Precio:</span> ${precio}`;

        contenedorServicios.appendChild(textoServicios);
        contenedorServicios.appendChild(precioServicios);
...
async function reservarCita(){
    ...
    if(resultado.resultado){
        ...
        icon: "error",
        title: "Error",
        text: "Hubo una falla al reserar la cita...",
        button: 'Ok'
        });
    };
